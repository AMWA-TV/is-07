...
The `connection_status_broker_topic` parameter holds the MQTT connection status topic and will always be set using the following format `x-nmos/connection/{id}`.  
...
MQTT publishers are required to send the `retained message` flag with every message (with the exception of the `connection_lost WILL` message) in order to solve the problem of late joiners.
